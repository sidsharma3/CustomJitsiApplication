...
class Endpoint @JvmOverloads constructor(
    ...
    private inner class TransceiverEventHandlerImpl : TransceiverEventHandler {
        ...
        override fun bandwidthEstimationChanged(newValue: Bandwidth) {
            logger.info { "YUIOP Estimated bandwidth is now $newValue" }
            logger.info { "ASDFG here is the Round trip time:" }
            logger.info { "ASDFG ${ getRtt() }" }
            logger.info { "${ getDebugState() }" }
            logger.info { "${ transceiver.getTransceiverStats() }" }
