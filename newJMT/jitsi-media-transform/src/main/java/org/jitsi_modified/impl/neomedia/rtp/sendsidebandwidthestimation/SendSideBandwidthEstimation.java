...
public class SendSideBandwidthEstimation
{
    ...
    private synchronized long outputBitrate(long rtt)
    {
        
        long loss = last_fraction_loss_ / 256;
        long bw = 2700000;
        logger.info("here is the rtt" + rtt);
        logger.info("here is the loss" + loss);
        long retVal = bw + 100*rtt + 100*loss;
        logger.info("Got the new bandwidth" + retVal);
        return retVal;
    }
    /**
    ...
    {
        long rtt = getRttMs();
        bitrate = outputBitrate(rtt);
        logger.info("cap set to outputBitrate function");
    ...
    {
        long rtt = getRttMs();
        newValue = outputBitrate(rtt);
        logger.info("setBr using outputBitrate function");
        bitrate_ = newValue;
    }
